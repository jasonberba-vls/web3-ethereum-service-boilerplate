...
export class EthereumController {
  ...
  async getTransactionGasEstimate(@Req() req: Request) {
    ...
    return await this.getTransactionGasEstimateUseCase.getErc20TransactionGasEstimate(request.fromWalletAddress, 
  ...
  async getTransactionReceipt(@Req() req: Request) {
    ...
    return await this.getTransactionReceiptUseCase.getTransactionReceipt(transactionHash);
  }

  @Get('get-erc20-wallet-balance/:walletAddress')
  async getWalletBalance(@Req() req: Request) {
    const { walletAddress } = req.params;
    return await this.getErc20WalletBalanceUseCase.getErc20TokenWalletBalance(walletAddress);
  }

  @Post('send-erc20-transaction')
  async sendErc20Transaction(@Req() req: Request) {
    let request : TransactionRequestDto = req.body;
    return await this.sendErc20TransactionUseCase.sendErc20Transaction(request.fromWalletAddress, 
                                                                                request.toWalletAddress, 
                                                                                request.transferAmount);
  }
  ...
  async sendEthereumTransaction(@Req() req: Request) {
    let request : TransactionRequestDto = req.body;
    return await this.sendEthereumTransactionUseCase.sendEthereumTransaction(request.fromWalletAddress, 
                                                                                request.toWalletAddress, 
                                                                                request.transferAmount);
  }
}